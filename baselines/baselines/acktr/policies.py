...
class ddrpDeepCnnPolicy(object):
    ...
    def __init__(self, sess, ob_space, ac_space, nenv, nsteps, nstack, reuse=False):
        ...
        with tf.variable_scope("model", reuse=reuse):
            h = conv(tf.cast(X, tf.float32)/11., 'c1', nf=64, rf=2, stride=1, init_scale=np.sqrt(2))
            h2 = conv(h, 'c2', nf=32, rf=3, stride=2, init_scale=np.sqrt(2))
            h3 = conv(h2, 'c3', nf=16, rf=2, stride=1, init_scale=np.sqrt(2))
            h4 = conv(h3, 'c4', nf=16, rf=2, stride=1, init_scale=np.sqrt(2))
            h5 = conv(h4, 'c5', nf=16, rf=2, stride=1, init_scale=np.sqrt(2))
            h5 = conv_to_fc(h5)
            h6 = fc(h5, 'fc1', nh=512, init_scale=np.sqrt(2))
            pi = fc(h6, 'pi', nact, act=lambda x:x)
            vf = fc(h6, 'v', 1, act=lambda x:x)
...
class ddrp2CnnPolicy(object):
    ...
    def __init__(self, sess, ob_space, ac_space, nenv, nsteps, nstack, reuse=False):
        ...
        with tf.variable_scope("model", reuse=reuse):
            h = conv(tf.cast(X, tf.float32)/11., 'c1', nf=64, rf=2, stride=1, init_scale=np.sqrt(2))
            h2 = conv(h, 'c2', nf=64, rf=2, stride=2, init_scale=np.sqrt(2))
            h2 = conv_to_fc(h2)
            h5 = fc(h2, 'fc1', nh=512, init_scale=np.sqrt(2))
...
class ddrpCnnPolicy(object):
    ...
    def __init__(self, sess, ob_space, ac_space, nenv, nsteps, nstack, reuse=False):
        ...
        X = tf.placeholder(tf.uint8, ob_shape) #obs
        with tf.variable_scope("model", reuse=reuse):
            h = conv(tf.cast(X, tf.float32)/11., 'c1', nf=32, rf=2, stride=1, init_scale=np.sqrt(2))
            h2 = conv(h, 'c2', nf=8, rf=2, stride=2, init_scale=np.sqrt(2))
            h3 = conv(h2, 'c3', nf=8, rf=2, stride=1, init_scale=np.sqrt(2))
            h3 = conv_to_fc(h3)
